...
class LearnForm(Form):
	...
	question = StringField('What is a question that differentiates your language from mine?')
...
def learn(id):
	...
	form = LearnForm()
	if form.validate_on_submit():
		game.expand(guess, form.language.data, form, form.answer.data == 'yes')
		return redirect(url_for('index'))
	return render_template('learn.html', guess=guess)
